...
module.exports = class extends Base {
  ...
  static useOneCard(game_info, sourcePlayer, card_index) {
    ...
    //为other_card_effect也指定出牌人Serial Number
  ...
  static useSkill(game_info, sourcePlayer) {
    ...
    if (sourcePlayer == undefined) {
      ...
      //先检查玩家本回合能不能使用技能
      if (sourcePlayer["skill_available"] == 0) {
        console.log(consoleSymbol["no"] + " 玩家本回合已经不能再使用技能了.");
        return game_info;
      }
      if (sourcePlayer["now_star"] >= c["card_cost"]) {
        //可以使用这张牌，玩家本回合费用减少
        sourcePlayer["now_star"] -= c["card_cost"];
        //指定玩家本回合已不能使用技能
        sourcePlayer["skill_available"]--;
        game_info['last_card'] = card_effect;
  ...
  static run_card_effect(game_info, card_effect) {
    ...
    if (c["other_card_effect"] != undefined) {
      console.log(consoleSymbol["info"] + " 递归结算卡牌或技能", c["other_card_effect"]);
  ...
  static game_condition(game_info, card_effect, condition_time) {
    ...
    if (game_info['condition'] == undefined) {
      console.log(consoleSymbol["no"] + " Why this condition is undefined?");
      game_info['condition'] = [];
      return game_info;
    }
    //针对condition列表进行结算
    ...
    if (time == 'begin') {
      ...
      for (let o in game_info["buff_list1"]) {
        ...
        if (eachBuff["settlement"] == 1 && player["id"] == nowTurn) {
          console.log(consoleSymbol["info"], " 己方回合开始进行buff结算", eachBuff);
      ...
      for (let o in game_info["buff_list2"]) {
        ...
        if (eachBuff["settlement"] == 2 && player["id"] == nowTurn) {
          console.log(consoleSymbol["info"], " 己方回合结束进行buff结算", eachBuff);
          game_info = this._buff(game_info, eachBuff);
        }
        //对方回合结束
  ...
  static _condition(game_info, card_effect, aim_card_effect, condition_time) {
    if (aim_card_effect == undefined) {
      ...
      if (card_effect["condition"]["condition_time"] == undefined || card_effect["condition"]["condition_time"] == 0) {
        console.log(consoleSymbol["no"] + " Debug: 执行一个静态奥秘判断,是否成功执行:", this._condition_static(game_info, card_effect), game_info);
  ...
  static _buff(game_info, card_effect) {
    ...
    if (c["buff_turn"] > 0) {
      ...
      console.log(consoleSymbol["no"], " 错误,有buff_turn为0的buff进入结算,检查清除buff的流程.", c);
    }
    // return game_info;
    /*
  ...
  static _normal(game_info, card_effect) {
    ...
    //对extra_function进行执行结算
    if (c["extra_function"] != undefined) {
      console.log(consoleSymbol["info"] + " 结算自定义函数功能", c["extra_function"]);
      try {
        //使用eval生成util函数
        util = new Function(c["extra_function"]);
        console.log("-----------------------------------------");
        console.log(util.toString());
        //调用util函数对game_info进行修改
        util.call({
          card_effect: c,
          game_info: game_info
        });
        ...
        console.log(consoleSymbol["no"] + " 执行自定义卡牌函数时出现严重错误", e);
      }
    }
  ...
  static _damage(game_info, player, damage) {
    //这里的pointer是指作用目标
    let lower = think.config('GameRules')["MinCuteValue"];
    let upper = think.config('GameRules')["CuteValueDefault"];
  ...
  static PlayerInfoInit(userid, socket_id, user_info) {
    if (user_info["skill"] == undefined)
      user_info["skill"] = {};
    if (user_info["skill_description"] == undefined)
      user_info["skill_description"] = "什么都不做.";
    let player = {};
    //因为创建玩家JSON的时候是已经匹配到对手之后，所以game_status = 1
    player['id'] = userid;
    player['socket'] = socket_id;
    player['status'] = 1;
    player["skill"] = user_info["skill"];
    player["skill_description"] = user_info["skill_description"];
    player["cute_value"] = think.config('GameRules')['CuteValueDefault'];
    ...
    player["tired"] = 1;
    if (user_info["type"] != undefined) {
      player["type"] = user_info["type"];
    } else {
      player["type"] = "玩家";
    }
    return player;
  ...
  static PlayerInfoFilter(game, p) {
    ...
    if (game["player1"]['id'] == p['id']) {
      ...
      player2 = {
        ...
        now_star: player["now_star"], //n in [0,12]
      ...
      //奥秘的掩盖处理
      for (let k of game["condition2"]) {
        if (k["source"] == undefined) {
          console.log(consoleSymbol["no"] + " 奥秘掩盖处理时无法找到source:", k);
        }
        cond = {
          "condition": {
            "secret": true
      ...
      for (let k of player["secret_condition_list"]) {
        if (k["source"] == undefined) {
          console.log(consoleSymbol["no"] + " 奥秘掩盖处理时无法找到source:", k);
        }
        cond = {
  ...
  static IsFinished(game_info) {
    ...
    if (game_info["player1"]["cute_value"] <= think.config("GameRules")["MinCuteValue"] && game_info["player2"]["cute_value"] > think.config("GameRules")["MinCuteValue"]) {
      winner = game_info["player2"];
      winner_serial_number = 2;
      game_info["game_status"]['FINISHED'] = winner;
      return winner;
    }
    if (game_info["player2"]["cute_value"] <= think.config("GameRules")["MinCuteValue"] && game_info["player1"]["cute_value"] > think.config("GameRules")["MinCuteValue"]) {
    ...
    //无胜利者
    //检查平局
    if (game_info["player1"]["cute_value"] <= think.config("GameRules")["MinCuteValue"] && game_info["player2"]["cute_value"] <= think.config("GameRules")["MinCuteValue"] || game_info['turns'] > think.config('GameRules')['MaxBattleTurns']) {
      return 'tie';
    }
    return false;
  }
  static async GameFinishedReward(game_info, game_status) {
    ...
    if (game_info["mode"] == undefined || game_info["mode"] == "PVP") {
      ...
      if (game_status == "tie") {
        ...
        await Player.addExp(game_info["player1"]["id"], battle_fail_exp);
        await Player.addExp(game_info["player2"]["id"], battle_fail_exp);
        await Player.addCoins(game_info["player1"]["id"], battle_fail_coins);
        await Player.addCoins(game_info["player2"]["id"], battle_fail_coins);
      } else {
        //有一个人赢了
        let winner = game_status;
        let loser = game_info["player1"]["id"] == winner["id"] ? game_info["player2"] : game_info["player1"];
        await Player.addExp(winner["id"], battle_succ_exp);
        await Player.addExp(loser["id"], battle_fail_exp);
        await Player.addCoins(winner["id"], battle_succ_coins);
        await Player.addCoins(loser["id"], battle_fail_coins);
      }
      ...
      await Player.AdventureCheckOut(game_info["player1"]["id"]);
    }
  }
};
